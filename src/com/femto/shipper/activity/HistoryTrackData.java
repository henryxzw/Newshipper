...
public class HistoryTrackData {
	...
	public class Points {
		...
		public double radius; // GPS定位精度
								// 当service的type是1，2，3，4，且创建该track的时候输入了这个字段才会返回。
	...
	public List<LatLng> getListPoints() {
		...
		while (it.hasNext()) {
			...
			if (Math.abs(location.get(0) - 0.0) < 0.01
					&& Math.abs(location.get(1) - 0.0) < 0.01) {
				continue;
			} else {
	...
	public String toString() {
		return "HistoryTrackData [status=" + status + ", size=" + size
				+ ", total=" + total + ", entity_name=" + entity_name
