...
public final class Response {
	...
	private Response(com.squareup.okhttp.Response response, int statusCode,
			String reqId, String xlog, String xvia, String address,
			double duration, String error, byte[] body) {
	...
	static Response create(com.squareup.okhttp.Response response,
			String address, double duration) {
		String error = null;
		...
		if (ctype(response).equals(Client.JsonMime)) {
			try {
				...
				if (response.code() >= 400 && !StringUtils.isNullOrEmpty(reqId)
						&& body != null) {
					ErrorBody errorBody = Json.decode(new String(body),
							ErrorBody.class);
		...
		return new Response(response, code, reqId, response.header("X-Log"),
				via(response), address, duration, error, body);
	}
	...
	public String toString() {
		return String
				.format(Locale.ENGLISH,
						...
						super.toString(), statusCode, reqId, xlog, xvia,
